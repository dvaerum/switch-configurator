...
### Fixed
- **Dry-run mode silently skipped FortiSwitch's pagination-disable commands** (`switch-configurator` 0.10.5): found while re-verifying the login fix below — `config system console` / `set output standard` (added in 0.10.1 to stop `show` commands hanging on FortiOS's `--More--` pager) weren't in either serial or SSH client's dry-run allow-list, so a `--dry-run` run skipped them and then hung on the very first multi-line `show` command anyway, defeating the 0.10.1 fix specifically in the one mode most used for safe testing. Both clients now execute these (like the existing `end`) even in dry-run.
- **A factory-reset (or otherwise policy-noncompliant) FortiSwitch could never log in at all** (`switch-configurator` 0.10.5): FortiOS demands a password change *during* the login exchange itself — a "New Password:"/"Re-enter New Password:" round trip tacked onto the normal login, with no way to opt out — which `SerialClient::login()` had no awareness of, so a factory-reset switch (blank/default admin password) permanently failed with a generic "not authenticated" error. `login()` now detects and completes this flow; a new `login_with_factory_default_fallback()` (used by FortiSwitch's serial connect path) retries once with a blank current password — FortiOS's single documented factory default, not a guessed credential — using the *configured* password as the new one if the device presents the forced-change prompt, bootstrapping the account to match the desired credentials in one pass. Verified against a real, freshly factory-reset FortiSwitch 108F-POE: first run bootstraps and applies; second run logs in normally and reports no changes needed.
...
- **FortiSwitch parser never converged after its first successful apply** (`switch-configurator` 0.10.3): found via the trace logging above, on the very first real convergence check after `IT-02876-sw1`'s first-ever successful apply. Three distinct real-hardware discrepancies, all in the new parser:
  - `show switch interface` compresses `allowed-vlans` into comma-separated ranges (`42,101-104,501`), not the space-separated individual ids the write side (`generate_port_commands`) sends. A bare whitespace split parsed the whole string as one invalid token and silently dropped every tagged VLAN on any trunk port. `parse_vlan_list` now expands both forms.
  - `show switch physical-port` never printed `set status`/`set poe-status` at all for ports already at this switch's own default — confirmed on hardware where every PoE-enabled copper port was missing both lines, permanently misread as PoE-*disabled* and endlessly "not converged". `PhysicalPortInfo`'s fields are now `Option`, and `build_ports` resolves an absent value against the model's own default (PoE-capable copper port → enabled; SFP+ uplink → disabled) instead of a single flat fallback.
  - `show switch interface` also lists FortiSwitch's own `internal` CPU/management interface alongside the real numbered ports. Treated as a stray configurable port, it made the diff engine want to "reset" it every cycle (this switch has `enforce_port_config: true`) even though it was never one of the 28 real ports. `build_ports` now only considers interfaces matching `port<number>`.
- **Removing an SNMP community from FortiSwitch was silently a no-op** (`switch-configurator` 0.10.4): the last discrepancy found while verifying the parser fixes above — `IT-02876-sw1` shipped with a factory-default `public` community the desired config never declared, and every apply correctly computed a removal diff for it, but `config system snmp community` / `delete public` did nothing (FortiOS's community list is index-based; `delete` doesn't accept a name), so the same "not converged" warning came back every cycle. `parse_current_state` now also records each community's numeric index, and removal deletes by that index instead of by name.
### Added
- **Persistent PoE on/off API endpoints** (`switch-configurator` 0.11.0): the only operational PoE control was `POST /switches/:id/poe-reset/:port_id`, a disable-wait-enable cycle that always ends powered-on — no way to leave a port off (or turn it back on) without a full reset. Requested by the provision-tool integration work for IT-02876 (DECT test setups need to power down unused base stations, not just power-cycle them). Adds `POST /switches/:id/poe-on/:port_id` and `POST /switches/:id/poe-off/:port_id`, reusing the same guard checks (`supports_poe`, `port_supports_poe`, vendor support, busy-check) and vendor `poe_enable_commands`/`poe_disable_commands` the reset endpoint already used — factored into a shared `poe_op_guard`/`poe_commands_for` so all three routes stay consistent. Progress streams over the existing `/api/events` SSE channel as a new `poe-set` event (`connecting`/`setting`/`done`/`failed` stages plus an `action: "on"|"off"` field), parallel to `poe-reset`'s event shape. Single-port only by design — the provision-tool UI loops over ports itself for bulk on/off across a setup's base stations; no bulk endpoint added for v1.
- **PoE reset support for FortiSwitch** (`switch-configurator` 0.10.5): the operational "Reset PoE" action was hardcoded to Aruba's CLI syntax and unconditionally rejected every other vendor, including FortiSwitch, with "PoE reset not yet supported" — even though the web UI's port table already showed the button on FortiSwitch PoE ports. `FortiswitchSwitch` gains `poe_disable_commands`/`poe_enable_commands` (`config switch physical-port` / `edit <port>` / `set poe-status disable|enable` / `next` / `end`), and the handler now dispatches on vendor instead of assuming Aruba. Verified end-to-end against real hardware (FortiSwitch 108F-POE) via the actual `poe-reset` API endpoint: disable → 3s wait → re-enable completed cleanly.
